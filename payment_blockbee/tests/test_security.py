...
class TestBlockBeeSecurity(PaymentCommon):
    ...
    def setUpClass(cls):
        ...
        cls.payment_method = cls.env.ref('payment_blockbee.payment_method_blockbee')
    ...
    def test_amount_currency_are_extracted_for_core_validation(self):
        tx = self._create_blockbee_tx()
        _order, data = self._create_webhook_data(tx)

        data['currency'] = tx.currency_id.name.lower()
        amount_data = tx._extract_amount_data(data)
    ...
    def test_api_key_is_sent_in_header(self):
