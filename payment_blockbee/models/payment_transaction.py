...
class PaymentTransaction(models.Model):
    ...
    def _blockbee_total_amount(self):
        """Return the full amount charged for this transaction.

        Odoo 16 supports provider checkout fees; core computes and stores them
        on the transaction at creation. The customer pays amount + fees, so
        both checkout creation and webhook validation must use this total.
        """
        self.ensure_one()
        return self.amount + (self.fees or 0.0)
    ...
    def _blockbee_payload(self):
        ...
        return {
            ...
            'order_total': Decimal(str(self._blockbee_total_amount())),
    ...
    def _process_notification_data(self, notification_data):
        ...
        # transaction because Odoo 16 allows the error -> done transition.
        # The expected value includes provider checkout fees.
        amount_data = self._parse_blockbee_amount_data(notification_data)
        expected_total = self._blockbee_total_amount()
        if (
            self.currency_id.compare_amounts(amount_data['amount'], expected_total) != 0
        ...
        # Odoo 16's _set_done already allows the cancel/error -> done
        # transitions, so no extra states are needed.
        self._set_done()
        if self.state != 'done':
